...
// TEST new url
const url = "https://www.laanonimaonline.com/"; /* https://previewlaol.laanonimaws.com/buscar?pag=1&clave=2885384 */
let to_convert = [[]];
...
async function la_scrap() {
    ...
    await article_codes_to_search.forEach(async (article_code_to_search) => {
        ...
        //Testing to solve error when codart doesnt exist on the website
        const selector_to_wait = "#maq_cuerpo > div.maq_col_2 > div.caja1.producto";
        const element_to_click = await page.waitForSelector(selector_to_wait, {timeout: 1500 }).catch(() => null);
        ...
        if(element_to_click){
            ...
            console.log(`Error: Element ${selector_to_wait} not found on the page. For the Article with code ${article_code_to_search}`);
            console.log(`Unable to click on the element.`)
        };
    });
};
